...
typedef struct {
  ...
  // // Added. if 'changed' is true, it means pre_bemps_free() is called (the memory usage of the Task is changed before the end of it)
  // changed is the amount of meomry will be freed after pre_bemps_free()
  long changed;
} bemps_beacon_t;
